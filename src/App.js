...
const SK = 'akash_hr_v2';
const COLS = ['sl','name','age','address','university','degree','jobMatch','phone','email','careerSummary'];
const LBL = {
...
function findColumns(headerRow) {
  ...
  if (!headerRow) return { nameCol: 2, careerCol: 3 };
...
function findDataStart(json) {
  for (let i = 0; i < Math.min(json.length, 20); i++) {
    const row = json[i] || [];
    const hasName = row.some(c => /^name$/i.test(String(c || '').trim()));
    const hasSL   = row.some(c => /^sl$/i.test(String(c || '').trim()));
    if (hasName && hasSL) return { headerIdx: i, dataStart: i + 1 };
  }
...
export default function App() {
  ...
  // University dropdown — unique sorted values from data
  ...
  }, [rows, search, uniFilter, ageMin, ageMax, jobMin, summaryFilter]);
  ...
  const handleFile = (file) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        const headerRow = headerIdx >= 0 ? json[headerIdx] : null;
        const { nameCol, careerCol } = findColumns(headerRow);
        ...
        if (!parsed.length) {
          setError(`No valid records found. Name col index: ${nameCol}, Career col index: ${careerCol}, data from row ${dataStart + 1}.`);
          setInfo(''); return;
        }
  ...
  // Export to Excel
  const exportExcel = () => {
    const exportCols = ['sl','name','age','address','university','degree','jobMatch','phone','email','careerSummary'];
    const data = [exportCols.map(c => LBL[c])];
    filtered.forEach(r => data.push(exportCols.map(c => r[c] || '')));
    const ws = XLSX.utils.aoa_to_sheet(data);
    ws['!cols'] = [5, 22, 6, 32, 36, 30, 12, 24, 28, 40].map(w => ({ wch: w }));
  ...
  const clearAll = () => {
    ...
    setAgeMin(''); setAgeMax(''); setJobMin(''); setSummary('All');
    ...
    <div className="app-wrapper">
      <div className="app">
        ...
        >
          ...
          <p className="sub">Reads Name &amp; Career Summary columns — skips all header rows automatically</p>
          <input
